...
There's an HDR profile that tone maps a 4K HDR10 file down to SDR, which is one of the heaviest jobs a media server ever does. You can also keep the HDR instead. The file just gets downscaled and stays proper HDR10 for clients that can display it. Keeping HDR needs an HEVC or AV1 output, and it's a much lighter job than tone mapping. On my iGPU it was actually slightly faster than a plain SDR transcode. AMD cards can transcode HDR perfectly well. What's missing on Linux is the driver's tone mapping filter, which is a driver gap rather than anything wrong with the hardware. Keeping HDR doesn't need that filter, so AMD cards can now downscale HDR files this way. There's a subtitle burn in toggle for the streaming test too, since burning subtitles into the picture is far more demanding than it sounds and it's worth knowing what it costs you.
